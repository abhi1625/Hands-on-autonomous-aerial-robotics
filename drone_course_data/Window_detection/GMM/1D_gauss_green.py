...
def generateData():
    stack = []
    for filename in os.listdir("data/green_train"):
        image = cv2.imread(os.path.join("data/green_train",filename))
...
while (cap.isOpened()):
 ...
 #   print(radius)
    ...
    cv2.imshow("Final output - Green buoy",test_image)
#    print(hull)
...
out = cv2.VideoWriter('1d_green.avi', fourcc, 5.0, (640, 480))
for v in video:
    out.write(v)
    cv2.waitKey(1)
