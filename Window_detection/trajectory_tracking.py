...
class trajectory_track:
	...
	def compute_ctrl_inputs(self, l_parallel, l_perpendicular):
		...
		ctrl_inputs_gf = l_parallel*ctrl_inputs[1] + l_perpendicular*ctrl_inputs[0]
...
def main():
	...
	while (not rospy.is_shutdown()):
		if init_flag :
			...
			init_flag = False
		track_ob.current_state = track_ob.current_state - np.array([bias_x,
																	bias_y,
																	bias_z,
																	bias_ang_x,
																	bias_ang_y,
																	bias_ang_z]) 
		print(track_ob.current_state[5])
		# print(track_ob.current_state)
		track_ob.next_des = np.array([1,1])
		...
		vel = Twist()
		vel.linear.x = 0.1*ctrl_inputs[0]
		vel.linear.y = 0.1*ctrl_inputs[1]
		vel.linear.z = 0
			
		vel.angular.x = 0
		vel.angular.y = 0
		vel.angular.z = 0.1*(yaw_reference - track_ob.current_state[5])
		print(vel.angular.z)
